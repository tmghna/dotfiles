...
if [ "$1" = "speaker" ]; then
  ...
  if wpctl inspect @DEFAULT_AUDIO_SINK@ | grep -iqE 'headphone|headset|earbud|bluez|usb'; then
    icon_active="<fn=1>󰋋</fn>" # md-headphones
    icon_muted="<fn=1>󰟎</fn>"  # md-headset_off
  else
    icon_active="<fn=2>󰕾</fn>" # md-volume_high
    icon_muted="<fn=2>󰝟</fn>"  # md-volume_off
  fi
...
# --- MICROPHONE LOGIC ---
  ...
  if echo "$mic_info" | grep -q "\[MUTED\]"; then
    ...
    echo "<fc=#F38BA8><fn=2></fn></fc>"
  else
    # Active Mic in Mauve (No text, just the icon)
    echo "<fc=#FAB387><fn=2></fn></fc>"
  fi
fi
